...
If you discover a security vulnerability in py-atmos-serial, please report it privately:

- **Preferred**: [GitHub private vulnerability reporting](https://github.com/marpi82/py-atmos-serial/security/advisories/new)
...
- **OpenSSF Scorecard**: weekly, via the Security Checks workflow
...
When using py-atmos-serial:

1. Keep dependencies updated (`uv sync --upgrade` in a controlled PR).
2. Run `uv run --group dev poe security` before releases.
3. Do not commit serial captures that contain installation-specific data.
4. The listener is passive. Do not point it at a port you do not own.
